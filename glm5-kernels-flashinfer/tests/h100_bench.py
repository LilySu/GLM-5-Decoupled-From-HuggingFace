...
Usage:
    ...
    python3 -m glm5-kernels-flashinfer.tests.h100_bench --mode bench

    # Full nsys profile (generates .nsys-rep file):
    nsys profile -o glm5_profile python3 -m glm5-kernels-flashinfer.tests.h100_bench --mode nsys

    # ncu kernel-level metrics (slow, targets specific kernels):
    ncu --set full --target-processes all -o glm5_ncu \\
        python3 -m glm5-kernels-flashinfer.tests.h100_bench --mode ncu

    # Multi-GPU (tensor parallel across N GPUs):
    torchrun --nproc_per_node=N -m glm5-kernels-flashinfer.tests.h100_bench --mode bench --tp N
...
def bench_flashinfer_decode(cfg, device):
    """Benchmark FlashInfer FA3 dense decode kernel."""
    try:
        from flashinfer.mla import BatchMLAPagedAttentionWrapper
    except ImportError:
        return BenchResult(name="flashinfer_decode", median_ms=-1, min_ms=-1, max_ms=-1,
                           num_iters=0, extra={"skip": "flashinfer not installed"})
    ...
    ckv = torch.randn(B * seq_len_kv, 1, d_ckv, dtype=torch.bfloat16, device=device)
    kpe = torch.randn(B * seq_len_kv, 1, d_kpe, dtype=torch.bfloat16, device=device)

    qo_indptr = torch.arange(0, B + 1, dtype=torch.int32, device=device)
    kv_indptr = torch.arange(0, B + 1, dtype=torch.int32, device=device) * seq_len_kv
    kv_indices = torch.arange(0, B * seq_len_kv, dtype=torch.int32, device=device)
    kv_lens = torch.full((B,), seq_len_kv, dtype=torch.int32, device=device)
    ...
    result.name = "flashinfer_decode"
    flops = 2 * B * H * seq_len_kv * (d_ckv + d_kpe + d_ckv)
    result.tflops = flops / (result.median_ms * 1e-3) / 1e12
    result.extra = {"batch": B, "seq_kv": seq_len_kv, "heads": H, "backend": "fa3"}
    return result
...
def generate_ncu_commands(script_path: str, output_prefix: str = "glm5_ncu"):
    ...
    # FlashInfer-only (filter by kernel name)
    commands["flashinfer"] = (
        f"ncu --metrics {quick_str} "
        f"--kernel-name 'flashinfer|batch_mla|trtllm' "
...
def main():
    ...
    # Run benchmarks
    components = {
        "flashmla": lambda: bench_flashinfer_decode(cfg, device),
