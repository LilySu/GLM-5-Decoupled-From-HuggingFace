...
Compares per-component and end-to-end performance across:
  ...
  3. glm5-kernels-flashinfer (CUDA kernels for MLA/DSA/MoE)
...
Requirements:
    ...
    - For kernel column: flashinfer and deep-gemm installed

Run:
    python3 -m glm5-kernels-flashinfer.tests.h100_bench_3way
    python3 -m glm5-kernels-flashinfer.tests.h100_bench_3way --full-dims
    python3 -m glm5-kernels-flashinfer.tests.h100_bench_3way --component mla
"""
...
def bench_rmsnorm(cfg, inputs, device):
    ...
    kern_rms = import_module("glm5-kernels-flashinfer.unsloth_rms_layernorm")
    kern_mla = import_module("glm5-kernels-flashinfer.mla_attention")
...
def bench_swiglu(cfg, inputs, device):
    ...
    # Kernels — same Triton kernel (reused from unsloth)
    kern_swiglu = import_module("glm5-kernels-flashinfer.unsloth_swiglu")
...
def bench_cross_entropy(cfg, inputs, device):
    ...
    # Kernels — same Triton kernel (reused from unsloth)
    kern_ce = import_module("glm5-kernels-flashinfer.unsloth_cross_entropy_loss")
...
def bench_rope(cfg, inputs, device):
    ...
    kern_rope = import_module("glm5-kernels-flashinfer.rope_partial")
...
def bench_moe_router(cfg, inputs, device):
    ...
    kern_router = import_module("glm5-kernels-flashinfer.moe_router")
...
def bench_dsa_indexer(cfg, inputs, device):
    ...
    kern_idx = import_module("glm5-kernels-flashinfer.dsa_indexer")
...
def bench_dsa_sparse_attn(cfg, inputs, device):
    """DSA sparse attention: mask+matmul (raw/triton) vs FlashInfer sparse kernel."""
    results = {}
    ...
    # Kernels — uses FlashInfer sparse if available, else same eager fallback
    kern_dsa = import_module("glm5-kernels-flashinfer.dsa_sparse_attention")
...
def bench_mla_attention(cfg, inputs, device):
    """MLA attention: PyTorch eager vs FlashInfer kernel."""
    results = {}
    ...
    with torch.no_grad():
        ...
        # Kernel (FlashInfer if available, else eager fallback)
...
def bench_moe_forward(cfg, inputs, device):
    ...
    triton_model = import_module("glm5-triton.model")
    kern_model = import_module("glm5-kernels-flashinfer.model")
...
def main():
    ...
    print("  With flashinfer/deep-gemm installed, MLA/indexer/MoE show real kernel speedup")
