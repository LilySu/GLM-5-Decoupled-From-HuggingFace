...
@skip_no_cuda
def h100_test_fp8_flashmla_kv_scale_correctness():
    """FlashMLA KV scales should be power-of-2 and correctly bracket the data range."""
    print("\n[H100-FP8-4] FlashMLA KV per-tile scale correctness")
    from importlib import import_module
    fp8 = import_module("glm5-kernels-flashmla-deepgemm.fp8_utils")
    import math
    ...
    # Create KV with known value ranges per tile
    kv = torch.zeros(1, 4, 1, 576, dtype=torch.bfloat16, device=device)
    kv[0, :, 0, 0:128] = 100.0     # tile 0: large values
    kv[0, :, 0, 128:256] = 0.01    # tile 1: small values
    kv[0, :, 0, 256:384] = 1.0     # tile 2: normal values
    kv[0, :, 0, 384:512] = -50.0   # tile 3: negative

    quantized = fp8.quantize_kv_flashmla(kv)
    raw = quantized.squeeze(2)
    scales = raw[..., 512:528].view(torch.float32)  # [1, 4, 4]

    ok = True
    for tile in range(4):
        s = scales[0, 0, tile].item()
        # Must be power of 2
        if s <= 0:
            print(f"  FAIL tile {tile}: scale={s} is not positive")
            ok = False
            continue
        log2_s = math.log2(s)
        if abs(log2_s - round(log2_s)) > 1e-6:
            print(f"  FAIL tile {tile}: scale={s} is not power of 2")
            ok = False
            continue
        # Scale should bracket the data: data/scale <= 448
        tile_max = kv[0, 0, 0, tile * 128:(tile + 1) * 128].abs().max().item()
        ratio = tile_max / s
        if ratio > 448.5:
            print(f"  FAIL tile {tile}: data/scale={ratio:.1f} > 448 (overflow risk)")
            ok = False

    if ok:
        print(f"  PASS all 4 tile scales are correct power-of-2 values that bracket data range")
    return ok
